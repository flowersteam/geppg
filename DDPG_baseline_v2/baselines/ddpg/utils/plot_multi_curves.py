...
data_folder = '/media/flowers/3C3C66F13C66A59C/data_save/ddpg_study_baseline/data/MountainCarContinuous-v0/figs/noise/'

gep=False

saving_name = 'noise'
legends=['No noise', 'Action pert.: OU ($\sigma=0.3$)','Action pert.: OU   $\sigma=0.6$','Parameter pert.']
...
for c, z ,i in zip(colors[:len(legends)], [0, 10, 20, 30, 40],[0,1,2,3, 4]):
    xs = np.arange(0,100,10)+4
    fig_tmp = plt.figure()
    # xs = []
    ...
    # xs=np.array(xs)
    ys = plt.hist(scores_our[i], range=[-5, 100])[0]
    plt.close()
    ...
    cs = [c] * len(xs)
    # cs[0] = 'c'
    ax.bar(xs, ys, zs=z, zdir='y', color=cs, alpha=0.8, width=7)
    ti=ax.get_xticks()
